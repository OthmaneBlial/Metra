...
fn legacy_tag_name(value: &str) -> Option<&'static str> {
    match value {
        ...
        "-GPSImgDirection" => Some("GPSImgDirection"),
        "-GPSSpeed" => Some("GPSSpeed"),
        "-GPSSpeedRef" => Some("GPSSpeedRef"),
        "-GPSTimeStamp" => Some("GPSTimeStamp"),
        "-GPSDateStamp" => Some("GPSDateStamp"),
    ...
    for assignment in entries {
            ...
            .or_else(|| key.strip_prefix("bext:"))
        {
            ...
            if name == "Comment" {
                ...
                options.push_id3_text(name, value);
            }
        } else {
            options.push_info(key, value);
        }
...
fn parse_edits(
    ...
    if let Some(key) = delete {
        ...
        if key != "JPEG:Comment" {
            if let Some(gps_key) = gps_decimal_key(key) {
                return Ok(Some(EditRequest::DirectTiff(vec![
                    metra::TiffEdit::DeleteGpsDecimal {
                        key: gps_key.to_owned(),
                    },
                ])));
            }
    ...
    match key {
        ...
        CopyKey::TiffGpsDecimal(key) => {
            ...
            let source_key = match key.as_str() {
                ...
                _ => {
                    eprintln!(
                        "metra: {}: unsupported GPS coordinate {key}",
                        source.display()
                    );
                    return ExitCode::from(1);
                }
        ...
        CopyKey::TiffGpsTime(key) => {
            ...
            let source_key = match key.as_str() {
                "GPS:GPSTimeStamp" => "GPS:TimeOfDaySeconds",
                _ => {
                    eprintln!("metra: {}: unsupported GPS time {key}", source.display());
                    return ExitCode::from(1);
                }
            };
            let Some(tag) = source_metadata.find(source_key) else {
            ...
            let edits = [metra::TiffEdit::SetGpsTime {
                key,
                value: value.to_string(),
            }];
            apply_tiff_edits(paths, &edits, limits)
