...
Only JPEG comment replacement/deletion/copy is implemented, through the
library API and the explicit `--set`/`--delete`/`--copy` CLI flags. The writer
reads and validates the source first,
