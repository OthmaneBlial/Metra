...
entries. `WavCreateOptions::with_ixml` adds one XML-validated iXML packet, while
`with_id3_text` and `with_id3_comment` add an optional bounded ID3v2 seed in an
`id3 ` chunk; the CLI accepts `iXML:Packet=VALUE`, `ID3:Title=VALUE`, and
`ID3:Comment=VALUE`. `WavCreateKind::Riff` uses classic 32-bit sizes; `Rf64` and `Bw64`
