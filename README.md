...
resources, plus existing AVI `LIST/INFO` strings through format-specific
rewrite APIs, and the CLI exposes the same narrow operations through `--set`,
`--delete`, and `--copy`.
...
tested; PSD XMP rewrites preserve the resource section size, and AVI INFO
rewrites preserve chunk sizes, then validate the result through their readers.
...
huit axes ci-dessous, arrondie à partir de 88,4 %, calculée uniquement sur le code
