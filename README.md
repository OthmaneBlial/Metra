...
deletion for existing ICC text tags stored as `desc` or `text` payloads; `mluc`
...
their existing storage; deletion zero-fills the payload so the tag disappears
on read. `mluc` replacement is intentionally not supported yet.
